...
import com.firesoftitan.play.titanbox.SlimefunSetup;
...
public abstract class AutomatedVanillaCraftingChamber extends SlimefunItem {
		...
		new BlockMenuPreset(name, "&eAutomated Vanilla Crafting Chamber") {
			...
			public void newInstance(final BlockMenu menu, final Block b) {
				...
				if (!BlockStorage.hasBlockInfo(b) || BlockStorage.getBlockInfo(b, "fill") == null || BlockStorage.getBlockInfo(b, "fill").equals("false")) {
					menu.replaceExistingItem(8, new CustomItem(new MaterialData(Material.GUNPOWDER), "&7Fill Empty: &4\u2718", "", "&e> Click to enable fill empty"));
	...
	protected void constructMenu(BlockMenuPreset preset) {
		...
		preset.addItem(2, new CustomItem(new MaterialData(Material.CRAFTING_TABLE), "&eRecipe", "", "&bPut in the Recipe you want to craft", "&4Vanilla Crafting Table Recipes ONLY"),
		new MenuClickHandler() {
	...
	private static boolean checkAndCraft(AutomatedVanillaCraftingChamber machine, Block b, BlockMenu menu, String myName) {
		if (SlimefunSetup.recipesV.containsKey(myName)) {
			ItemStack output = SlimefunSetup.recipesV.get(myName);
