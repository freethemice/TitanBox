    ...
    protected void tick(Block b) {
        try {
            if (ChargableBlock.isChargable(b)) {
                ...
                if (!processing.containsKey(b)) {
                    ...
                    for (int slot : getInputSlots()) {
                        if (bedRock.getType() == Material.BEDROCK) {
                            if (SlimefunManager.isItemSimiliar(BlockStorage.getInventory(b).getItemInSlot(slot), SlimefunItemsHolder.BEDROCK_DRILL, true)) {
                                MachineRecipe r = new MachineRecipe(4 / getSpeed(), new ItemStack[]{SlimefunItemsHolder.BEDROCK_DRILL.clone()}, new ItemStack[]{new ItemStack(Material.COBBLESTONE)});
                                startProcess(b, slot, r);
                                proccess = true;
                                break;
                            }
                            if (SlimefunManager.isItemSimiliar(BlockStorage.getInventory(b).getItemInSlot(slot), SlimefunItemsHolder.BEDROCK_DRILL_OLD, true)) {
                                MachineRecipe r = new MachineRecipe(4 / getSpeed(), new ItemStack[]{SlimefunItemsHolder.BEDROCK_DRILL_OLD.clone()}, new ItemStack[]{new ItemStack(Material.COBBLESTONE)});
                                startProcess(b, slot, r);
                                proccess = true;
                                break;
                            }
                ...
                if (SlimefunManager.isItemSimiliar(WhatAmIDoing.getInput()[0], me.mrCookieSlime.Slimefun.Lists.SlimefunItems.REACTOR_COOLANT_CELL, true))
                {

                }
                if (SlimefunManager.isItemSimiliar(WhatAmIDoing.getInput()[0], SlimefunItemsHolder.BEDROCK_DRILL, true) || SlimefunManager.isItemSimiliar(WhatAmIDoing.getInput()[0], SlimefunItemsHolder.BEDROCK_DRILL_OLD, true)) {
                    adding = new ItemStack(Material.COBBLESTONE, 1);
                    if (SlimefunStartup.chance(1000, 1)) adding = SlimefunItemsHolder.BEDROCK_DUST.clone();
                }
