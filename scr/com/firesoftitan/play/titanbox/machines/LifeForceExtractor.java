...
public abstract class LifeForceExtractor extends PickThreeContainer {;
    public LifeForceExtractor(Category category, ItemStack item, String name, RecipeType recipeType, ItemStack[] recipe) {
    ...
    protected void tick(Block b) {
        ...
        if (isProcessing(b)) {
            ...
            if (timeleft > 0 && getSpeed() < 10) {
                ...
                pushItems(b, new ItemStack[] {SlimefunItemsManager.JAR_OF_LIFE_FORCE.clone()});
            ...
            for (int slot: getInputSlots()) {
                if (!Utilities.isEmpty(BlockStorage.getInventory(b).getItemInSlot(slot))) {
                    if (Utilities.isItemEqual(BlockStorage.getInventory(b).getItemInSlot(slot), SlimefunItemsManager.EMPTY_JAR)) {
                        BlockStorage.getInventory(b).replaceExistingItem(31, new CustomItem(new ItemStack(Material.CLOCK), "Searching for Life Force...."));
                        ...
                        if (location.getBlock().getType() == Material.NETHERRACK) {
                            ItemStack adding = new ItemStack(Material.GRAVEL, 1);
                            sendForProcessing(b, location, adding, slot);
                        }
                        if (location.getBlock().getType() == Material.NETHER_BRICKS) {
                            ItemStack adding = new ItemStack(Material.BRICKS, 1);
                            sendForProcessing(b, location, adding, slot);
                        }
                        if (location.getBlock().getType() == Material.NETHER_BRICK_FENCE) {
                            ItemStack adding = new ItemStack(Material.OAK_FENCE, 1);
                            sendForProcessing(b, location, adding, slot);
                        }
                        if (location.getBlock().getType() == Material.NETHER_BRICK_SLAB) {
                            ItemStack adding = new ItemStack(Material.STONE_BRICK_SLAB, 1);
                            sendForProcessing(b, location, adding, slot);
                        }
                        if (location.getBlock().getType() == Material.NETHER_BRICK_STAIRS) {
                            ItemStack adding = new ItemStack(Material.STONE_BRICK_STAIRS, 1);
                            sendForProcessing(b, location, adding, slot);
                        }
                        return;
                    }
