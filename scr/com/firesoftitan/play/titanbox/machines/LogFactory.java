            ...
            for (int slot: getInputSlots()) {
                if (!Utilities.isEmpty(BlockStorage.getInventory(b).getItemInSlot(slot))) {
                    if (BlockStorage.getInventory(b).getItemInSlot(slot).getType().name().toLowerCase().endsWith("_sapling")) {
                        ...
                        ItemStack adding = new ItemStack(Material.OAK_LOG, 10 + number.nextInt(10));
                        for (TreeTypeEnum treeTypeEnum : TreeTypeEnum.values()) {
                            if (treeTypeEnum.getSapling().equals(saplingType)) {
                                adding = new ItemStack(treeTypeEnum.getLog(), 3 + number.nextInt(10));
                            }
                        }
                        adding = adding.clone();
